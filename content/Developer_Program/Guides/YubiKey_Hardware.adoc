...
- link:/OTP/[OTP (Touch triggered One Time Passwords)]
- link:/OATH/[OATH OTPs (6/8 digit OTPs, like as seen with Google Authenticator)]
- link:/PGP/[OpenPGP Smart Card]
- link:/PIV/[PIV Smart Card]
- link:/WebAuthn/[FIDO U2F/WebAuthn Authenticator]

On the YubiKey, each Application has a dedicated memory space within the secure cryptographic element, and no Application can access data stored in any of the others. For more details on what data can be stored in the YubiKey, refer to link:/Developer_Program/Guides/User_Loaded_Data.html[YubiKey User-Loaded Data].
