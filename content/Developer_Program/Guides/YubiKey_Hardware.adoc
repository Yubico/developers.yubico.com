...
== What is a YubiKey?
A YubiKey is a multi-protocol multi-factor hardware authenticator, providing strong authentication to a wide range of services and situations. While YubiKeys come in a number of different form-factors, each are built around the same core chipset and firmware, allowing a uniform experience regardless of the model used.
...
The YubiKey Management Application provides the serial number and YubiKey firmware version to the other Applications, as well as being able to turn on or off access to the other functions over the physical port or NFC communication channels. Further, the YubiKey Management Application can be secured with a 6 byte lock code, preventing unauthorized modification to the YubiKey.
