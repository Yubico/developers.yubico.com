...
Using `yubihsm-shell`, the attestation functionality can be leveraged to produce a selfsigned X509Certificate that can
...
# Import the attestation certificate to use it as a template when signing the selsigned certificate. Use the same ObjectID as the generated key
...
# Delete the template certificate to make room for the selfsigned certificate to be imported
$ yubihsm-shell -a delete-object -i <KEY_OBJECT_ID> -t opaque

# Import the selfsigned certificate using the same ObjectID as the generated key
