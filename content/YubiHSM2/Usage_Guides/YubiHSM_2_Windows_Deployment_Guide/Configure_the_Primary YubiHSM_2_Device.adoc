...
_Note: When replacing the default authentication key using this setup, the new authentication key will only be operable in the same domain as the asymmetric key. This is due to the https://developers.yubico.com/YubiHSM2/Concepts/Domain.html[domain concept] that is used to compartmentalize the YubiHSM 2 device._
