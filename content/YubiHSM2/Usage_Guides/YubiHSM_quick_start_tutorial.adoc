...
Assuming we have a file called `data.txt` containing the data we would like to sign, and a file called `data.sha` that contains the SHA-256 message digest of the data, we will sign using ECDSA with the Asymmetric key we generated in the previous step
