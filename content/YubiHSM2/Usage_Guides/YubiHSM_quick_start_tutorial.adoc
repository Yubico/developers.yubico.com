...
Before reading this document you should get familiar with concepts such as: `Sessions`, `Domains`, `Capabilities` described in the link:../Concepts[Concepts] Section.
...
Add an Authkey with ID `2` belonging to all possible *Domains*, with all *Capabilities* enabled and with password `password`.
  ...
  yubihsm> put authkey 0 2 0xffff 0xffffffff password
...
Note how the command above has two distinct sets of Capabilities. This is because Authkey Objects also have link:../Concepts/Capability.adoc[Delegated Capabilities].

List all object to see the newly created object

  yubihsm> list objects 0 2 any 0 0 any
...
We now proceed to generate a new key type, Asymmetric. In our example we will use this key to sign some data and we will also export this key _under wrap_ to another HSM for backup purposes.
