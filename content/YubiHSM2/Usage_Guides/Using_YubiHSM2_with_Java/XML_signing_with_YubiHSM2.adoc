...
We can now use `xmlsectool` to validate an XML digital signature using the public key in the signing certificate:
...
In case the signature does not validate, `xmlsectool` will complain:
