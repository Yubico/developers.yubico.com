...
For this procedure to work you must have https://www.gnupg.org[GnuPG]
version 2.0.22 or later installed on your computer.
The version of the YubiKey's OpenPGP module must be
1.0.5 or later. To check this version you may run, after inserting your YubiKey:
...
You'll also need the YubiKey's link:/PGP/[Admin PIN].
...
After this the keyring is saved. And that point it no longer contains the real secret key,
only a pointer indicating that it's stored on a smart card.
