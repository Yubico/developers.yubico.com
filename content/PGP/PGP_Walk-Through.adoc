...
For OpenPGP reference, see link:https://www.openpgp.org/[OpenPGP] and download site, see link:https://www.gnupg.org/download/index.html[GnuPG].
...
* Yubico Manager, to ensure that the operating system recognizes the Yubico device as a smart card.

The Yubico Manager is available for Windows, OSX, and Linux. Installers can be downloaded from the Yubico website using the links listed at:
...
* A Yubico smart card that holds a private key.
