...
The *PIV* option is easier for those who have no reason to use the same key for both authentication and signing. The PIV option also offers the opensc-pkcs11 module, or Yubico's own ykcs11 module, which is a lighter software package than the the OpenSC software.
