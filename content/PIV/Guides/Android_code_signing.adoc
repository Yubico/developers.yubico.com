...
+
If the keytool command fails, adding the -debug option might help understand the problem. One common issue is that the slots might be indexed differently (sarting from 0, for example), requiring the file created on the step above to be configured in another way.
