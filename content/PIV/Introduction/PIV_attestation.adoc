...
The YubiKey comes with a pre-loaded attestation certificate signed by a link:/PKI/yubico-piv-ca-1.pem[Yubico PIV CA]. This can be overwritten by loading a new key and certificate to slot f9. After the Yubico key is overwritten it can not be brought back. The attestation key and certificate will not be cleared out by a reset of the device.
