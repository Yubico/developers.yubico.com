== YubiKey-enabled iOS App Registration Process
...
The <<Actions Required By Yo,Actions Required By You>> section provides detailed instructions for each step in the approval process. Submitting an app to the App Store without the Yubico 5Ci MFi PPID will result in rejection from Apple.

=== UI Components for YubiKey iOS Apps
Download the https://github.com/YubicoLabs/yubikit-ios/releases[SDK.zip] file for a list of free-to-use example assets for use in your YubiKey-enabled iOS application. Under the extracted folder: 
...
For approval, the app must meet the following guidelines:
 ...
 - User Experience: The app must communicate YubiKey status to users in an acceptable fashion determined by Yubico.
   ...
   * Touch YubiKey 5Ci Animation Guideline
     ** If the YubiKey is inserted when the application requests an operation, display the touch key animation.
...
1. Register App with Yubico - Register your app by filling out our https://partners.yubico.com/prm/English/c/lightning-partner-preview-support/[Yubico App Registration Form]. Include the following:
...
Contact For questions or comments not covered by this document please open a https://support.yubico.com/support/tickets/newhttps://support.yubico.com/support/tickets/new[developer support ticket].
