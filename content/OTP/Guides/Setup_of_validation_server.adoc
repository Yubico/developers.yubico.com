== Setup of a Yubico OTP validation server
...
For testing purposes we will be using `curl`, which we also need to install:

  $ sudo apt-get install curl

Lastly, we will be programming a YubiKey for testing purposes, and for that we
will need `ykpersonalize`:

  $ sudo apt-get install yubikey-personalization

=== Installing the KSM
For the Key Storage Module we will use the python-pyhsm project, which can be
used with or without a YubiHSM device. First, we install the required packages:
