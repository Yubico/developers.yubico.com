...
OpenSSH with support for FIDO2 credentials is available on Linux, but not yet on Windows. MacOS does not currently support FIDO2 credentials in the bundled version of OpenSSH, but this feature may be enabled via installing OpenSSH via homebrew.

== Discoverable vs Non-Discoverable Credentials
Before configuring an OpenSSH server or Client for FIDO2 credentials, the decision must be reached as whether to use Discoverable or Non-Discoverable credentials. Either option has different strengths, and the best option depends on the environment SSH is being used in.
