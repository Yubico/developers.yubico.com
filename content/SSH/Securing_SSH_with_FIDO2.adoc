...
+
NOTE: Working in WSL2? The OpenSSH inside WSL2 cannot reach a USB security key directly. See link:/SSH/Securing_SSH_and_git_with_FIDO2_on_WSL2.html[Securing SSH and Git Commit Signing with FIDO2 on WSL2] for a setup that routes WSL2 through the Windows FIDO components.
+
Alternatives:
+
