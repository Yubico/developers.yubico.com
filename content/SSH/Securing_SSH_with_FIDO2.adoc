...
- `ssh-agent` can remember the reference so you don’t have to re-specify the key file each time.  
- `ssh-agent` does **not** cache your FIDO2 PIN. If your key requires a PIN, you will be prompted for it whenever the key is used.  
- If the key was created with `-O verify-required` or has `credProtect` set, you will still need to physically touch the YubiKey on every authentication attempt.  
