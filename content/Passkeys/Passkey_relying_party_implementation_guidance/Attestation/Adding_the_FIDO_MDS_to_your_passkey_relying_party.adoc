...
Figure 4 demonstrates how to load the BLOB and create a `FidoMetadataService` object. First, call `loadCachedBlob()` on the downloader to retrieve the metadata BLOB, then pass it to the `FidoMetadataService` builder.
