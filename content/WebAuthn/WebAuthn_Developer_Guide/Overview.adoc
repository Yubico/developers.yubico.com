...
FIDO2 introduces the concept of a discoverable key credential, also know as a resident key, which is/are stored on the authenticator instead of encrypted and stored on the server. Discoverable keys also enable passwordless authentication scenarios, at the expense of consuming limited storage space on the authenticator.
