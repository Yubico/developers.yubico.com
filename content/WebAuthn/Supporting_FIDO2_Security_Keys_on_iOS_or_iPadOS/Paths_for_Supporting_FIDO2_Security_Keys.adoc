== Paths for Supporting FIDO2 Security Keys

The rest of this article offers general guidance on which path to take for supporting FIDO2 security keys in the Apple ecosystem.
...
The Web Application route utilizes the Safari browser to register and authenticate users with FIDO2 security keys.
...
*Option #2*. Use the Yubico iOS SDK to support YubiKey protocols other than FIDO2. This path is also for developers who want more control over the WebAuthn flow.To get started, check out the link:https://github.com/Yubico/yubikit-ios[Yubico Mobile iOS SDK].
