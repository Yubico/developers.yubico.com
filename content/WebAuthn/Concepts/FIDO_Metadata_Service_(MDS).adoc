...
Your FIDO Server will need to download the MDS directly from the FIDO Alliance. The Yubico java-webauthn-server has built in capabilities to download the repository. The following Java code sample can be used to initialize the MDS in your application.
...
Next you will get the Trust Root Certificates for the MDS. You can proceed with the default method using useDefaultTrustRoot() while building your downloader. Next is useTrustRootCacheFile - the FIDO Spec notes that when grabbing the Trust Root Certificate, your app should have some sort of caching logic. Our example above is writing this to a file in a directory named "tmp"
...
The next step will be to load the BLOB and create a FidoMetadataService object. First, call `loadCachedBlob()` on the downloader to retrieve the metadata BLOB, then pass it to the FidoMetadataService builder:
----
MetadataBLOB blob = downloader.loadCachedBlob();
...
FidoMetadataService.builder()
  .useBlob(blob)
  .prefilter(entry -> 
    entry.getMetadataStatement()
      .map(ms -> ms.getAttachmentHint()
        .map(hints -> hints.contains(AttachmentHint.ATTACHMENT_HINT_INTERNAL))
        .orElse(false))
      .orElse(false))
  .build();
----
...
== Convenience Metadata Service
The FIDO Alliance also provides a link:https://fidoalliance.org/specs/mds/fido-convenience-metadata-service-v1.0-ps-20250521.html[Convenience Metadata Service (C-MDS)], separate from the security-focused MDS BLOB. This service is designed specifically for building user-friendly credential management interfaces by providing:

* Human-readable authenticator display names (`friendlyNames`) with localization support
* Authenticator icons (`icon`, `iconDark`)
* Provider logos (`providerLogoLight`, `providerLogoDark`)

This data is useful for showing users which authenticators they've registered, without mixing security metadata with display concerns.

=== Accessing the Convenience Metadata Service
The service is available at https://c-mds.fidoalliance.org/ and returns a JSON object keyed by AAGUID:

----
{
  "7409272d-1ff9-4e10-9fc9-ac0019c124fd": {
    "friendlyNames": {"en-US": "YubiKey Bio Series"},
    "icon": "data:image/png;base64,..."
  },
  "d7781e5d-e353-46aa-afe2-3ca49f13332a": {
    "friendlyNames": {"en-US": "YubiKey 5 Series with NFC"},
    "icon": "data:image/png;base64,..."
  }
}
----

NOTE: The Convenience Metadata Service is intended for UI display purposes only. Continue using the main MDS BLOB for attestation validation and security decisions.
