...
The sample below will demonstrate how to filter for only authenticators that are FIDO Certified for Level 1.

----
FidoMetadataService.builder()
  .useDownloader(downloader)
  .filter(blobEntry => 
    blobEntry.getStatusReports.stream()
      .anyMatch(statusReport => statusReport.getStatus == AuthenticatorStatus.FIDO_CERTIFIED_L1))
  .build()
----
