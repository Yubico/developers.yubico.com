...
* link:/Developer_Program/WebAuthn_Starter_Kit/Attestation.html[WebAuthn Attestation and Authenticator Metadata]
...
Finally, it could also be used together with link:/PIV/Introduction/PIV_attestation.html[PIV Attestation] to prove that a FIDO2 credential and a PIV certificate both originate and reside on the same device, based on the fact that both attestations share the same serial number.
