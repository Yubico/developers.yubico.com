...
Client hints are optional values sent by the relying party (RP) to help the browser decide which authenticators to show first in the UI. They aim to improve the user experience in passkey flows. Support for client hints varies by browser and platform.
...
For details and code examples, see the link:../WebAuthn_Developer_Guide/WebAuthn_Client_Hints.html[Client Hints section].
