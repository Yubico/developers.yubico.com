...
^.^|NFC a|N/A a|N/A a|N/A a|N/A
|===
*Safari is not able to set a PIN for User Verification if one is not already set.  Requests to create a credential that requires User Verification may appear to succeed, but create a credential that does not require a PIN.
