...
NOTE: This process is not required for Android applications or for iOS/iPad apps not interacting with the YubiKey 5Ci. For example, interaction with a YubiKey over NFC or USB-C DOES NOT require Apple MFi approval. This is only for interaction with the Lightning connector of the YubiKey 5Ci.
