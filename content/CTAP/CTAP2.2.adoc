...
Spec:: Extends the hmac-secret extension to support invocation during credential creation (MakeCredential), in addition to authentication (GetAssertion).
...
| Yubico.NET SDK | ✅ Extensive Support | Supports PPUAT, thirdPartyPayment, hmac-secret-mc, PIN complexity policies, and extended getInfo properties.
...
| libfido2 / fido2-token | ✅ Command-line Support | Lower-level C library with tools supporting getInfo querying and PIN management.
