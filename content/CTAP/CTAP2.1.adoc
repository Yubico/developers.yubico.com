...
The `fido2-token` command-line tool, part of the Yubico-led link:/libfido2/[libfido2] project, is used here to illustrate CTAP 2.1 features.
...
To list relying parties having discoverable credentials stored on a device:
...
To list stored discoverable credentials for a given relying party:

....
$ fido2-token -L -k ssh: $DEV
Enter PIN: ******
00: bH95gDocVbWZtAzHcyZhHi1LLmh6gYl857P3ffraUzTDfe1/dqgTPMDS+awITlxC openssh AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA= eddsa uvreq
....

To delete a discoverable credential, use the `-D` option and specify its base64-encoded credential ID with `-i`:

....
$ fido2-token -D -i bH95gDocVbWZtAzHcyZhHi1LLmh6gYl857P3ffraUzTDfe1/dqgTPMDS+awITlxC $DEV
Enter PIN: ******
....
...
See also our guidance on link:/WebAuthn/Concepts/Enterprise_Attestation/[using Enterprise Attestation in web applications].
